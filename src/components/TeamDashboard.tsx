...
import { Edit3, Users, Briefcase, BarChart3, Check, Loader2, ArrowRight, UserCheck, Hammer } from 'lucide-react';
        ...
        {teams.map((team) => (
          ...
          >
            ...
            <div className="p-6 bg-slate-50/50 dark:bg-slate-800/20 flex flex-col gap-2">
               ...
               {/* BOTÃO DE DEPLOY (MARCADO COMO OFFLINE/DEV) */}
               {onDeployProject && (
                  <button 
                     onClick={() => onDeployProject(team)}
                     className="w-full bg-slate-200 dark:bg-slate-800 text-slate-500 dark:text-slate-400 py-3 rounded-xl text-xs font-black flex items-center justify-center gap-2 border border-slate-300 dark:border-slate-700 hover:bg-slate-300 dark:hover:bg-slate-700 transition-colors mb-2"
                     title="Funcionalidade em Desenvolvimento"
                  >
                     <Hammer size={16} /> DEPLOY AUTOMATIZADO (DEV)
                  </button>
               )}
