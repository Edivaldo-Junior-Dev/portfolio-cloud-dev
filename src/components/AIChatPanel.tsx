...
const AIChatPanel: React.FC<AIChatPanelProps> = ({ proposals, members, votes }) => {
  ...
  const [activeSubTab, setActiveSubTab] = useState<'visual' | 'doc' | 'ai-scoring' | 'text' | 'export'>('doc'); // Default to doc as per request
  ...
  const handlePrint = () => {
    if (reportRef.current) {
        ...
        // Fallback: Imprimir documento de texto
        const docText = generateReportText(votes, members, proposals);
        const win = window.open('', '', 'height=700,width=900');
        if(win) {
            win.document.write('<html><head><title>Documento Oficial</title>');
            win.document.write('<style>body { font-family: monospace; white-space: pre-wrap; padding: 20px; }</style>');
  ...
  const handleDownloadDoc = () => {
     ...
     a.href = url;
     a.download = `matriz_analise_oficial_${new Date().toISOString().slice(0,10)}.md`;
  ...
  const handleCopyDoc = () => {
    ...
    alert('Documento Oficial copiado para a área de transferência!');
  };
      ...
      <div className="flex flex-col bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden shadow-lg">
        ...
        <div className="flex border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 overflow-x-auto">
             <button 
                ...
                className={`flex-1 min-w-[100px] py-3 text-xs font-semibold flex items-center justify-center gap-2 transition-colors ${activeSubTab === 'doc' ? 'bg-white dark:bg-slate-800 border-b-2 border-orange-500 text-orange-600 dark:text-orange-400' : 'text-slate-500 hover:text-orange-500'}`}
            >
                <FileType size={14} /> Documento Oficial
            </button>
            <button 
                    ...
                    <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-4 shadow-sm flex-1 flex flex-col">
                         <div className="flex justify-between items-center mb-4 border-b border-slate-100 dark:border-slate-700 pb-3">
                             <h3 className="font-bold text-slate-800 dark:text-white flex items-center gap-2">
                                ...
                                Modelo "Matriz de Análise Comparativa"
                             </h3>
                             <div className="flex gap-2">
                                 <button onClick={handleCopyDoc} className="text-xs bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-600 dark:text-white px-3 py-1.5 rounded flex items-center gap-1 transition-colors">
                                     <Share2 size={12} /> Copiar Texto
                                 </button>
                                 <button onClick={handleDownloadDoc} className="text-xs bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-300 px-3 py-1.5 rounded flex items-center gap-1 hover:bg-orange-200 dark:hover:bg-orange-800/40 transition-colors">
                         ...
                         <textarea 
                            readOnly 
                            value={generateReportText(votes, members, proposals)} 
                            className="w-full flex-1 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg p-4 font-mono text-xs leading-relaxed text-slate-700 dark:text-slate-300 resize-none outline-none focus:ring-2 focus:ring-orange-500/50"
                         />
                    </div>
                 </div>
            )}
